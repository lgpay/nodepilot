...
- [x] 列表 / 详情 / 启停 / 删除
- [x] 在线状态（心跳判定）
- [ ] 资源看板（CPU / 内存 / xray 状态）— 当前版本不采集 CPU/内存（字段保留兼容，默认 0）；`xray_running` 未入库
- [x] 一键安装命令生成（含预分配节点 token）— `GET /nodes/:id/install` + Web 节点详情页展示可复制命令
- [x] 节点有效期（`expires_at`）：到期自动停用（`enabled=false` + 下线）+ 心跳拒绝（403）+ `node_expired` 通知
...
- [x] **格式（format）**：vmess(V2Ray, base64) / clash(YAML) / surfboard(Surge 语法，见 §7.3) / loon(.conf) / sip008(JSON)
...
- [x] ACL4SSR：内置 14 个 rule-provider（.list）+ 11 个标准分组 + 16 条路由规则（含内置 GEOIP,CN / FINAL，Clash 用 MATCH 替代 FINAL）；Loon 用内置最小规则（GEOIP,CN,DIRECT / FINAL）
- [x] 订阅内容随配置 / 自愈端口变更自动更新（端口取自 client 所属 inbound 当前 port）

### 4.6 配置下发 — P0（核心）
- [x] 自动 / 手动下发 `POST /nodes/{id}/config/sync`（自动触发：入站保存且启用 TLS 时自动下发；**用户增删改不自动下发，需手动触发**）
- [x] 管理端生成 xray JSON → `PUT /agent/v1/config` → agent 写盘 → 重启 xray 热重载（MVP 用进程重启实现 reload，非 API reload）
- [x] 版本管理 `GET /nodes/{id}/config/versions`（auto-sync 自动补发）
- [ ] 一键回滚到历史版本 — P0 未实现（版本记录已存，缺回滚接口与按钮）
- [x] 失败回执标红 — Web 版本表对 `failed` 状态做红色高亮（index.html）
- 注：节点月流量控制（见 4.2）由 15 分钟定时扫描器（`StartAlertScheduler`）驱动；证书随下发自动分发（见 4.9）
...
- [x] 事件：节点离线（心跳超时 / 自愈耗尽）、节点已自愈（换端口恢复）、节点恢复在线、节点到期停用、节点月流量预警 / 耗尽、流量超额、客户端到期 / 临近到期（提前 3 天）（共 9 个事件，见 `notify.NotifyEvents`）
...
- [x] CF Token AES-GCM 加密存储；证书私钥随分发下发至节点本地（落盘 `0600`），管理端签发产物权限收紧为 `0600`（注：需求早期「私钥不跨机」描述不准确，见 §7.2）

### 4.10 运维 / 安装 — P0/P1
- [x] 节点一键安装脚本（xray + agent + systemd）— `scripts/install-agent.sh`（交互 / 非交互 `NP_SERVER/NP_TOKEN/NP_NODE_ID`），Web 节点详情页生成安装命令
- [x] 管理端备份（SQLite 在线快照）— `scripts/backup.sh`；恢复仍需人工替换数据库文件
- [ ] agent 自更新：P2 未实现
...
- [x] 订阅页（P1）
- [x] 统计页（P1）
- [x] 设置页 — 基础版已实现（心跳间隔、自动下发与主题等）；通知渠道管理在侧边栏「通知」内
...
Node(id, name, address "ip:port", region, tags[],
     token_hash, token_enc, enabled, status[online|offline],
     connectivity[ok|degraded|offline], agent_version, cpu, mem,
     last_heartbeat, port_range TEXT,        -- 如 "10000-65535" 或 "10000-20000,30000-40000"；NULL=用全局默认
     monthly_traffic_bytes, expires_at)      -- 月流量上限(0=不限) / 节点有效期(空=长期)

Inbound(id, node_id, protocol, port, transport,
        tls{cert_id, enabled}, stream_settings, fallback,
        enabled, port_auto_fixed, auto_heal, auto_heal_interval)

Client(id, inbound_id, uuid, alias,
       traffic_limit_bytes, expire_time, enabled)

SubscriptionGroup(id, name, token(AES 密文), format[vmess|clash|surfboard|loon|sip008],
                  mode[none|acl4ssr_online],
                  filters{node_ids[], protocol[], tags[], inbound_ids[]}, enabled)
...
SelfHealLog(id, node_id, attempts, last_action, result, time)  -- 未实现：自愈状态在内存态 + inbound.port_auto_fixed 标记，无持久化表

NotificationChannel(id, type[email|wecom|tg], name, enabled, config, events, created_at)

Setting(key="default_port_range"|"panel_base_url"|"acl4ssr_base", value="10000-65535")
```
...
| GET | `/nodes/{id}/install` | Admin | 生成节点一键安装命令（含解密 token） |
| GET | `/nodes/{id}/connectivity` | Admin | 手动连通测试（未实现，自动探测调度已具备） |
| GET | `/nodes/{id}/selfheal/log` | Admin | 自愈记录（未实现，自愈状态在内存） |
| POST/GET | `/subscriptions` | Admin | 建 / 列 / 删分组 |
| GET/PATCH | `/subscriptions/{id}` | Admin | 详情 / 改筛选格式 |
| GET | `/sub/{token}` | token | 对外订阅端点（客户端拉取） |
| GET | `/qr/{token}` | token | 订阅二维码 PNG |
| POST | `/nodes/{id}/heartbeat` `/nodes/{id}/traffic` | Node Token | 节点上报状态 / 流量 |
...
| GET | `/stats/overview` | Admin | 流量统计聚合（今日 / 节点 / 客户端 / 30 天趋势） |
| GET | `/geo` | Admin | IP 归属查询（自动识别节点区域） |
| GET/POST/PATCH/DELETE | `/notifiers...` | Admin | 通知渠道 CRUD + 测试 |
| GET/PUT | `/settings/default_port_range` | Admin | 全局默认端口范围（未实现，仅 `store.Setting` 内部读写） |
| GET | `/rules/:name` | 公开 | ACL4SSR 规则镜像（rule-provider / RULE-SET 引用） |
...
|------|------|------|
| GET | `/health` | 存活探测 |
| GET | `/status` | agent 版本 + xray 运行状态 + 配置路径 |
| GET | `/config` | 当前生效配置 + 版本（未实现） |
| PUT | `/config` | 接收下发的 xray config JSON（写盘 + 重启 xray 进程热重载） |
| POST | `/config/reload` | 热重载 xray（未实现，当前由 PUT /config 直接重启进程） |
| GET | `/traffic` | 当前流量明细（未实现，流量由 agent 周期主动上报控制面） |
...
### 7.2 证书申请与匹配
管理端统一用 lego 走 Cloudflare DNS-01 申请**泛域名**证书 → 产物存管理端 `/opt/nodepilot/certs/`（签发后 `.crt`/`.key` 权限收紧为 `0600`）→ 经 `PUT /agent/v1/cert` 分发到所有已启用节点（证书落节点本地 `/opt/nodepilot-agent/certs/`，私钥 `0600`）→ DB（`Certificate`）仅存节点端路径与元数据 → inbound 用 `cert_id` 引用 → 生成 xray 时填入 `tlsSettings.certificates`；泛域名证书可多入站复用；管理端到期前 30 天定时续签并自动重分发。CF Token AES-GCM 加密存储。**私钥随分发下发至节点本地**（分发是跨机传输，管理端仅保留收紧权限后的签发产物）。
...
- `clash`=YAML；`surfboard`=**Surge 语法** .conf（Surfboard 官方不解析 Clash YAML 的 `proxies:` 段，必须输出 Surge 格式）；`mode=acl4ssr` 时附加 ACL4SSR 的 14 个 rule-provider + 11 分组 + 16 条路由规则；
...
- **触发点**：`probe.go` 在节点离线（`notifyOffline`）、自愈成功（`notifyHealed`）、状态切回 ok（`notify.Dispatch("node_recovered")`，每恢复仅一次）、节点到期（`node_expired`）调用；`alert.go` 的 15 分钟扫描器处理节点月流量预警/耗尽与客户端流量超额、到期/临近到期，按 `client:原因:日期` 每日起重去重防刷屏。
